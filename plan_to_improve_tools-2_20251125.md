...
### 1. `tools/ppt_format_shape.py` v3.0

**Key Features:**
- Color presets matching `ppt_add_shape.py` (primary, secondary, accent, etc.)
- Transparency/opacity support with presets (subtle, light, medium, etc.)
- Transparency accepts percentages ("30%") and decimals (0.3)
- Shape info before/after formatting
- Presentation version tracking
- Color contrast validation with warnings
- Text formatting within shapes (color, size, bold)
- "transparent" and "none" to clear fill
- Comprehensive CLI with examples
- Proper error handling for all v3.0 exception types
...
### 2. `tools/ppt_validate_presentation.py` v3.0

**Key Features:**
- Three built-in policies: lenient, standard, strict
- Custom threshold overrides via CLI
- Slide-by-slide issue breakdown
- Actionable fix commands for every issue
- Design rule validation (6x6 rule when enforced)
- Font count tracking
- Issue severity classification (critical, warning, info)
- Pass/fail based on policy thresholds
- Policy violations list
- Prioritized recommendations with fix tools
- Summary statistics
- Integration with System Prompt v3.0 validation gates
...
| Accessibility | Missing alt text, low contrast, small text |
| Assets | Large images, large file size |
| Design | Font count, 6x6 rule (strict policy) |

**Policy Thresholds:**
| Threshold | Lenient | Standard | Strict |
|-----------|---------|----------|--------|
| max_critical_issues | 10 | 0 | 0 |
| max_empty_slides | 5 | 0 | 0 |
| max_missing_alt_text | 20 | 5 | 0 |
| require_all_alt_text | No | No | Yes |
| enforce_6x6_rule | No | No | Yes |
| max_fonts | 10 | 5 | 3 |
...
- Full alignment with PowerPoint Agent Core v3.0
- Consistent JSON output structure
- Version tracking (tool and core versions)
