# Implementation Plan - Powerpoint Agent Tools Guide Update

I have performed a meticulous analysis of the codebase state. We currently have **30 functional tools** spanning creation, manipulation, inspection, and export. The existing guide is outdated (covering only the initial P0/P1 set) and lacks the depth required for an "Elite AI Presentation Architect."

## Re-imagining the Guide
I will transform `powerpoint_agent_tools_guide.md` from a simple list into a **Technical Reference Manual**.

### Key Structural Changes:
1.  **Operational Theory:** Explicitly define the "Stateless Inspection Pattern" (Inspect -> Target -> Edit) which is crucial for agents to successfully modify existing slides.
2.  **Complete Catalog:** Organize all 30 tools into 8 logical domains (Architecture, Slides, Text, Media, Vector Graphics, Data, Inspection, Output).
3.  **Data Structures:** Document the exact JSON schemas for Charts, Tables, and Positioning, as these are the most common failure points for LLMs.
4.  **Workflow Recipes:** Provide complex, multi-step recipes (e.g., "The Corporate Rebrand", "The Monthly Report") that combine tools.

I will now generate the complete, validated `powerpoint_agent_tools_guide.md`.

***

# powerpoint_agent_tools_guide.md
