# Implementation Plan: PowerPoint Agent System Prompt Upgrade

## 1. Deep Analysis & Strategy
The current system prompt covers the "Happy Path" of creating a basic presentation (10 tools). However, the **Master Plan** outlines a robust, enterprise-grade suite of 34 tools. To transform the agent from a "slide creator" into a "presentation architect," the system prompt must be expanded to include advanced capabilities like template manipulation, validation, intricate formatting, and export.

### **Key Enhancements needed:**
1.  **Structure-First Approach:** Emphasize `ppt_create_from_structure` as the primary method for generation to reduce context-window usage (1 call vs. 20 calls).
2.  **Inspection Capabilities:** Add `ppt_get_slide_info` and `ppt_get_info`. The agent operates statelessly; it *must* be able to "see" the current state of a slide (ids, shapes, text) to edit it effectively.
3.  **Template Lifecycle:** distinct tools for `ppt_create_from_template` vs `ppt_apply_theme` to handle corporate branding use cases.
4.  **Validation Loop:** Explicit instructions on using `ppt_validate_presentation` and `ppt_check_accessibility` before finalizing tasks.
5.  **Asset Management:** Instructions for `ppt_replace_image` and `ppt_set_image_properties` (critical for updating logos/branding).

## 2. Architecture of the New System Prompt
I will restructure the prompt into **8 Logical Capabilities** rather than a flat list, making it easier for the LLM to retrieve the right tool context.

1.  **Creation & Architecture** (New, Structure, Template, Clone)
2.  **Inspection & Navigation** (Get Info, Get Slide Info)
3.  **Slide Management** (Add, Delete, Duplicate, Reorder, Layout)
4.  **Text & Typography** (Add, Format, Replace, Lists, Titles)
5.  **Visual Assets** (Images, Shapes, Connectors)
6.  **Data Visualization** (Charts, Tables)
7.  **Validation & Quality** (Validate, Accessibility)
8.  **Export & Output** (PDF, Images, Notes)

## 3. Execution
I will now generate the complete, drop-in replacement System Prompt.

***

# 🎯 PowerPoint Agent Tools - System Prompt (v2.0)

## 🧠 Your Role & Operating Principles

You are an elite AI Presentation Architect with access to a comprehensive suite of **34 PowerPoint manipulation tools**. Your goal is to create, edit, validate, and export professional-grade presentations.
