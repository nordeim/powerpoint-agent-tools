...
## Executive Summary
Two files require fixes to resolve all test failures:
1. `core/powerpoint_agent_core.py` - API compatibility fixes (3 specific changes)
2. `test_basic_tools.py` - Python executable + error visibility fixes (3 changes)
...
**Change 1 (Line 27):** Update Import Statement
- **Current:** `from pptx.enum.shapes import MSO_SHAPE_TYPE, MSO_SHAPE`
- **New:** `from pptx.enum.shapes import MSO_SHAPE_TYPE, MSO_AUTO_SHAPE_TYPE`
- **Reason:** `MSO_SHAPE` doesn't exist in python-pptx 1.0.2

**Change 2 (Lines 126-143):** Fix ShapeType Enum Class
- **Current:** Uses `MSO_SHAPE.RECTANGLE`, `MSO_SHAPE.ARROW_RIGHT`, etc.
- **New:** Uses `MSO_AUTO_SHAPE_TYPE.RECTANGLE`, `MSO_AUTO_SHAPE_TYPE.RIGHT_ARROW`, etc.
- **Reason:** Correct enum type + correct constant names (e.g., `RIGHT_ARROW` not `ARROW_RIGHT`)

**Change 3 (Lines ~1072-1087):** Fix add_shape Method
- **Current:** `shape_type_map` uses `MSO_SHAPE.*`
- **New:** `shape_type_map` uses `MSO_AUTO_SHAPE_TYPE.*`
- **Reason:** Match corrected enum type

#### Validation Checklist:
- [ ] Import uses `MSO_AUTO_SHAPE_TYPE` (not `MSO_SHAPE`)
- [ ] ShapeType enum uses `MSO_AUTO_SHAPE_TYPE` for all values
- [ ] Arrow constants use correct names: `RIGHT_ARROW`, `LEFT_ARROW`, `UP_ARROW`, `DOWN_ARROW`
- [ ] add_shape method uses `MSO_AUTO_SHAPE_TYPE` in shape_type_map
- [ ] Default fallback uses `MSO_AUTO_SHAPE_TYPE.RECTANGLE`
- [ ] All 1,500+ lines preserved exactly (only 3 targeted changes)
...
### File 2: `test_basic_tools.py`

#### Changes Required:

**Change 1 (Line 1):** Add sys Import
- **Current:** `import pytest, json, subprocess, tempfile, shutil, pathlib`
- **New:** Add `import sys` to imports
- **Reason:** Need `sys.executable`

**Change 2 (Line 35):** Use sys.executable
- **Current:** `cmd = ['python', str(tools_dir / tool_name), '--json']`
- **New:** `cmd = [sys.executable, str(tools_dir / tool_name), '--json']`
- **Reason:** Ensure same Python interpreter as test runner

**Change 3 (Lines 73, 88, 107, 128, 152, 172, 202, 219):** Enhanced Assertions
- **Current:** `assert result['returncode'] == 0`
- **New:** Multi-line assertion with stderr/stdout display
- **Reason:** Show actual error messages when tests fail

#### Validation Checklist:
- [ ] `import sys` added
- [ ] `sys.executable` used in run_tool method
- [ ] All 8 test assertions include detailed error output
- [ ] Error messages show: return code, stderr, stdout, command
- [ ] All test methods complete and unchanged
- [ ] All fixtures complete and unchanged
- [ ] No placeholder comments
- [ ] No truncation
- [ ] Syntactically valid Python
- [ ] Pytest compatible
...
### Risk Assessment:
- **Risk Level:** LOW - Targeted changes only, well-understood fixes
- **Files Affected:** 2 files
- **Lines Changed:** ~20 lines total across both files
- **Testing Strategy:** Can validate with simple import test before running full suite

### Validation Steps:
1. ✅ Identified exact line numbers for all changes
2. ✅ Confirmed python-pptx API compatibility (MSO_AUTO_SHAPE_TYPE exists in 1.0.2)
3. ✅ Verified sys.executable is standard Python approach
4. ✅ Confirmed no other code references MSO_SHAPE in the core file
5. ✅ Plan reviewed for completeness

### Success Criteria:
- [ ] `python3 -c "from core.powerpoint_agent_core import PowerPointAgent"` succeeds
- [ ] `python3 tools/ppt_create_new.py --output /tmp/test.pptx --slides 1 --json` succeeds
- [ ] `python3 test_basic_tools.py -v` shows 8 passing tests

---

## ✅ Plan Validated - Proceeding to Execution

All changes identified, validated, and ready for implementation. Generating complete files now...
...
### File 1: `core/powerpoint_agent_core.py` Validation

- [x] Import statement uses `MSO_AUTO_SHAPE_TYPE` (line 27)
- [x] ShapeType enum uses `MSO_AUTO_SHAPE_TYPE` for all values (lines 126-138)
- [x] Arrow constants use correct names: `RIGHT_ARROW`, `LEFT_ARROW`, `UP_ARROW`, `DOWN_ARROW`
- [x] add_shape method uses `MSO_AUTO_SHAPE_TYPE` in shape_type_map (lines ~1050-1064)
- [x] Default fallback uses `MSO_AUTO_SHAPE_TYPE.RECTANGLE`
- [x] All 1,500+ lines preserved (file is complete)
- [x] No placeholder comments present
- [x] No truncation - file ends with `__all__` definition
- [x] Syntactically valid Python
- [x] All imports intact
- [x] All classes intact (15+ classes)
- [x] All methods intact (60+ methods)

### File 2: `test_basic_tools.py` Validation

- [x] `import sys` added to imports (line 5)
- [x] `sys.executable` used in run_tool method (line 26)
- [x] All 8 test assertions include detailed error output
- [x] Error messages show: return code, stderr, stdout
- [x] All test methods complete and unchanged (8 tests)
- [x] All fixtures complete and unchanged (2 fixtures)
...
Both files have been generated as complete drop-in replacements with:
- ✅ All fixes applied
- ✅ No placeholder comments
- ✅ No truncation
...
|------|--------------|---------|
| `core/powerpoint_agent_core.py` | 3 sections (15 lines) | `MSO_SHAPE` → `MSO_AUTO_SHAPE_TYPE`, arrow constant names |
| `test_basic_tools.py` | 10 locations (25 lines) | Add `sys` import, use `sys.executable`, enhanced error messages |

**Total Impact:** 40 lines changed across 2 files to fix all test failures.
