...
- **Grid System**: 12-column grid positioning
  ...
  ```
- **Excel Reference**: Cell-based positioning
  ```json
  {"grid": "C4"}
  ```
...
| `ppt_delete_slide.py`        | `--file PATH` (req), `--slide N` (req)        | Remove slide (destructive; requires explicit approval)  |
| `ppt_duplicate_slide.py`     | `--file PATH` (req), `--slide N` (req)        | Clone slide to end of deck                               |
...
| `ppt_add_text_box.py`        | `--file PATH` (req), `--slide N` (req), `--text TEXT`, `--position JSON`, `--size JSON`, `--font-name NAME`, `--font-size N`, `--color HEX`, `--alignment {left,center,right,justify}` | Add free-floating formatted text            |
| `ppt_add_bullet_list.py`     | `--file PATH` (req), `--slide N` (req), `--items "A,B,C"`, `--position JSON`, `--size JSON`, `--bullet-style {bullet,numbered,none}` | Add structured lists (enforces 6×6 rule)    |
| `ppt_format_text.py`         | `--file PATH` (req), `--slide N` (req), `--shape N` (req), `--font-name NAME`, `--font-size N`, `--color HEX`, `--bold`, `--italic` | Apply styling to existing text              |
| `ppt_search_and_replace_text.py`| `--file PATH` (req), `--find TEXT`, `--replace TEXT`, `--match-case`, `--dry-run` | Global text replacement (preview first)     |

### 🖼️ **Domain 4: Images & Media**
| Tool                         | Critical Arguments                                          | Purpose                                     |
|------------------------------|------------------------------------------------------------|---------------------------------------------|
| `ppt_add_image.py`           | `--file PATH` (req), `--slide N` (req), `--image PATH` (req), `--position JSON`, `--size JSON` (use `"width": "auto"` to maintain aspect ratio), `--alt-text TEXT`, `--compress` | Insert image with accessibility compliance  |
...
| Tool                         | Critical Arguments                                          | Purpose                                     |
|------------------------------|------------------------------------------------------------|---------------------------------------------|
| `ppt_add_shape.py`           | `--file PATH` (req), `--slide N` (req), `--shape TYPE` (req), `--position JSON`, `--size JSON`, `--fill-color HEX`, `--line-color HEX` | Add geometric elements: `rectangle`, `rounded_rectangle`, `ellipse`, `triangle`, `arrow_right`, `star` |
| `ppt_update_shape.py`        | `--file PATH` (req), `--slide N` (req), `--shape-index I` (req), `--properties JSON` | Modify existing shapes                      |
...
| `ppt_set_master_theme.py`    | `--file PATH` (req), `--theme-file PATH` (req)            | Apply corporate theme                       |
| `ppt_replace_color.py`       | `--file PATH` (req), `--old-color HEX`, `--new-color HEX` | Rebrand color scheme                        |
| `ppt_set_background.py`      | `--file PATH` (req), `--slide N` (optional), `--color HEX`, `--image PATH` | Set slide background                        |

### 📊 **Domain 6: Data Visualization**
| Tool                         | Critical Arguments                                          | Purpose                                     |
|------------------------------|------------------------------------------------------------|---------------------------------------------|
| `ppt_add_chart.py`           | `--file PATH` (req), `--slide N` (req), `--chart-type {column,bar,line,pie,scatter}` (req), `--data PATH` (req) or `--data-string JSON`, `--position JSON`, `--size JSON`, `--title TEXT` | Add charts from JSON data |
| `ppt_run_chart_update.py`    | `--file PATH` (req), `--slide N` (req), `--chart-index I` (req), `--data-file PATH` | Refresh existing chart data                 |
...
| `ppt_add_table.py`           | `--file PATH` (req), `--slide N` (req), `--rows N` (req), `--cols N` (req), `--data PATH`, `--headers "A,B,C"`, `--position JSON`, `--size JSON` | Insert data tables |
...
| `ppt_get_shape_thumbnail.py` | `--file PATH` (req), `--slide N` (req), `--shape-index I` (req), `--output PATH` (req) | Generate thumbnail of specific shape       |
...
| `ppt_export_png.py`          | `--file PATH` (req), `--output PATH`, `--slides [N]` | Export slides as high-res images                         |
| `ppt_export_images.py`       | `--file PATH` (req), `--output-dir PATH`, `--format {png,jpg}` | Export all slides as images in specified format         |
| `ppt_export_pdf.py`          | `--file PATH` (req), `--output PATH` (req) | Convert deck to PDF (Requires LibreOffice)              |
| `ppt_set_slide_notes.py`     | `--file PATH` (req), `--slide N` (req), `--notes TEXT` | Set speaker notes for a slide                            |
...
2. **Readability Overlay** (for text on images):
   ...
   ppt_add_shape.py --file PATH --slide N --shape rectangle \
     ...
     --style '{"fill": {"color": "#000000", "alpha": 0.35}, "line": {"visible": false}}' \
     --json
   ```
3. **Content Emphasis**: 
...
5. **Layering for Readability**: Add semi-transparent rectangle behind text on busy backgrounds:
   ...
   ppt_add_shape.py --file PATH --slide N --shape rectangle \
     ...
     --style '{"fill": {"color": "#FFFFFF", "alpha": 0.85}, "line": {"visible": false}}' \
     --json
   ```
6. **Image Handling**: When inserting images, use `"width": "auto"` in size parameter to maintain aspect ratio:
   ```bash
   ppt_add_image.py --file PATH --slide N --image image.jpg \
     --position '{"left": "10%", "top": "20%"}' \
...
ppt_run_chart_update.py --file report_week_42.pptx --slide 1 --chart-index 0 \
  --data-file new_revenue.json --json

# 4. Update title date
ppt_set_title.py --file report_week_42.pptx --slide 0 \
  --title "Weekly Report - Week 42" --json

# 5. Validation
ppt_validate_presentation.py --file report_week_42.pptx --json
...
ppt_replace_image.py --file deck.pptx --slide 0 --old-image "Logo_Old" \
  --new-image new_logo.png --json

# 3. Shape recolor (loop through slides and shapes)
ppt_format_shape.py --file deck.pptx --slide 0 --shape 1 --fill-color "#00B050" --json
...
- [ ] Destructive operations (`ppt_delete_slide.py`) approved explicitly by user
- [ ] Full command history logged with JSON outputs
- [ ] Color scheme matches canonical palettes or user-specified branding
- [ ] All images have appropriate alt text set via `ppt_set_image_properties.py`
- [ ] Images maintain proper aspect ratio using `"width": "auto"` parameter
- [ ] Optional parameters correctly marked and handled (e.g., `--slide N` in background settings)
